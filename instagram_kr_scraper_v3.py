#!/usr/bin/env python3
"""
Instagram 韩系内容深度抓取分析脚本 - V3 男士场景扩展版
新增男士运动垫场景：力量训练、HIIT、俯卧撑、平板支撑
"""

import asyncio
import json
import random
from datetime import datetime
from typing import Dict, List, Any

# 原有账号列表
HOME_ACCOUNTS = [
    "todayhouse", "_home_", "homestyling_kr", "yenni.hom", 
    "fantastic_table", "iriskim.cozy", "casa__home", 
    "monologue__home", "home__ing_", "our92square"
]

YOGA_ACCOUNTS = [
    "yoga_korea", "home_training_kr", "yoga__journey", 
    "pilates_kr", "yogini_kr"
]

# 新增男士健身账号
MALE_FITNESS_ACCOUNTS = [
    "hometraining_kr_men", "fitness_kr_men", "men_yoga_kr"
]

# 原有标签
TAGS = [
    "한국인테리어", "홈스타일링", "요가", "홈트레이닝", 
    "요가매트", "홈요가", "남성요가", "여성요가", 
    "명상", "웰니스", "필라테스", "욜홈", "요가일기", "요가소품"
]

# 男士瑜伽标签
MALE_YOGA_TAGS = ["남성요가", "남자요가", "요가남"]

# 女士瑜伽标签
FEMALE_YOGA_TAGS = ["여성요가", "여자요가", "요가걸"]

# 新增男士运动垫标签
MALE_FITNESS_TAGS = [
    "울트라매트", "트레이닝매트", "홈트매트", 
    "남자홈트", "헬스매트", "피트니스매트"
]

class InstagramAnalyzer:
    def __init__(self):
        self.data = {
            "metadata": {
                "collected_at": datetime.now().isoformat(),
                "total_posts": 0,
                "accounts_scraped": [],
                "tags_scraped": []
            },
            "home_accounts": {},
            "yoga_accounts": {},
            "male_fitness_accounts": {},
            "tags_data": {},
            "male_yoga": {
                "posts": [],
                "color_schemes": {},
                "layouts": {},
                "props": {},
                "lighting": {"natural": 0, "artificial": 0},
                "exercise_types": {},
                "mat_types": {}
            },
            "male_fitness": {
                "posts": [],
                "color_schemes": {},
                "layouts": {},
                "props": {},
                "lighting": {"natural": 0, "artificial": 0},
                "exercise_types": {},
                "mat_types": {}
            },
            "female_yoga": {
                "posts": [],
                "color_schemes": {},
                "layouts": {},
                "props": {},
                "lighting": {"natural": 0, "artificial": 0},
                "exercise_types": {},
                "mat_types": {}
            },
            "overall": {
                "color_schemes": {},
                "layouts": {},
                "props": {},
                "lighting": {"natural": 0, "artificial": 0},
                "yoga_mat_colors": {},
                "room_styles": {},
                "time_of_day": {},
                "exercise_types": {},
                "mat_types": {}
            }
        }
        self.total_posts = 0
        
    def add_post(self, post: Dict, category: str):
        """添加单条内容并分析"""
        self.total_posts += 1
        
        target_data = None
        if category == "male_yoga":
            target_data = self.data["male_yoga"]
        elif category == "male_fitness":
            target_data = self.data["male_fitness"]
        elif category == "female_yoga":
            target_data = self.data["female_yoga"]
        
        # 分析颜色
        for color in post.get("colors", []):
            self.data["overall"]["color_schemes"][color] = self.data["overall"]["color_schemes"].get(color, 0) + 1
            if target_data:
                target_data["color_schemes"][color] = target_data["color_schemes"].get(color, 0) + 1
        
        # 分析布局
        layout = post.get("layout", "")
        if layout:
            self.data["overall"]["layouts"][layout] = self.data["overall"]["layouts"].get(layout, 0) + 1
            if target_data:
                target_data["layouts"][layout] = target_data["layouts"].get(layout, 0) + 1
        
        # 分析道具
        for prop in post.get("props", []):
            self.data["overall"]["props"][prop] = self.data["overall"]["props"].get(prop, 0) + 1
            if target_data:
                target_data["props"][prop] = target_data["props"].get(prop, 0) + 1
        
        # 分析光线
        lighting = post.get("lighting", "natural")
        if lighting == "natural":
            self.data["overall"]["lighting"]["natural"] += 1
            if target_data:
                target_data["lighting"]["natural"] += 1
        else:
            self.data["overall"]["lighting"]["artificial"] += 1
            if target_data:
                target_data["lighting"]["artificial"] += 1
        
        # 瑜伽垫/运动垫颜色
        mat_color = post.get("mat_color", "")
        if mat_color:
            self.data["overall"]["yoga_mat_colors"][mat_color] = self.data["overall"]["yoga_mat_colors"].get(mat_color, 0) + 1
        
        # 房间风格
        style = post.get("style", "")
        if style:
            self.data["overall"]["room_styles"][style] = self.data["overall"]["room_styles"].get(style, 0) + 1
        
        # 时间段
        time_of_day = post.get("time_of_day", "")
        if time_of_day:
            self.data["overall"]["time_of_day"][time_of_day] = self.data["overall"]["time_of_day"].get(time_of_day, 0) + 1
        
        # 运动类型
        exercise = post.get("exercise_type", "")
        if exercise:
            self.data["overall"]["exercise_types"][exercise] = self.data["overall"]["exercise_types"].get(exercise, 0) + 1
            if target_data:
                target_data["exercise_types"][exercise] = target_data["exercise_types"].get(exercise, 0) + 1
        
        # 垫子类型
        mat_type = post.get("mat_type", "")
        if mat_type:
            self.data["overall"]["mat_types"][mat_type] = self.data["overall"]["mat_types"].get(mat_type, 0) + 1
            if target_data:
                target_data["mat_types"][mat_type] = target_data["mat_types"].get(mat_type, 0) + 1
    
    def generate_report(self) -> str:
        """生成详细分析报告"""
        lines = []
        
        lines.append("# Instagram 韩系瑜伽垫场景深度分析报告 V3\n")
        lines.append(f"\n**数据收集时间**: {self.data['metadata']['collected_at']}\n")
        lines.append(f"**总样本数**: {self.total_posts} 条\n")
        lines.append(f"**抓取账号数**: {len(HOME_ACCOUNTS)} 个家居账号 + {len(YOGA_ACCOUNTS)} 个瑜伽账号 + {len(MALE_FITNESS_ACCOUNTS)} 个男士健身账号\n")
        lines.append(f"**分析标签数**: {len(TAGS)} 个核心标签 + {len(MALE_YOGA_TAGS)} 个男士瑜伽标签 + {len(FEMALE_YOGA_TAGS)} 个女士瑜伽标签 + {len(MALE_FITNESS_TAGS)} 个男士健身标签\n")
        
        # ========== 整体配色方案统计 ==========
        lines.append("\n---\n")
        lines.append("\n## 一、整体配色方案统计\n")
        lines.append("\n### 1.1 配色方案排名（整体）\n")
        lines.append("| 排名 | 颜色 | 出现次数 | 占比 | 趋势 |\n")
        lines.append("|------|------|----------|------|------|\n")
        total_colors = sum(self.data["overall"]["color_schemes"].values())
        sorted_colors = sorted(self.data["overall"]["color_schemes"].items(), key=lambda x: -x[1])
        for i, (color, count) in enumerate(sorted_colors[:15], 1):
            pct = (count / total_colors * 100) if total_colors > 0 else 0
            trend = "🔥热门" if pct > 15 else "📈上升" if pct > 8 else "➡️稳定"
            lines.append(f"| {i} | {color} | {count} | {pct:.1f}% | {trend} |\n")
        
        # ========== 男士场景深度分析 ==========
        lines.append("\n---\n")
        lines.append("\n## 二、男士场景深度分析\n")
        
        # 男士瑜伽场景
        lines.append("\n### 2.1 男士瑜伽场景\n")
        male_yoga_total = sum(self.data["male_yoga"]["color_schemes"].values())
        lines.append(f"**样本数**: {male_yoga_total} 条\n")
        
        lines.append("\n#### 配色偏好\n")
        lines.append("| 颜色 | 出现次数 | 占比 |\n")
        lines.append("|------|----------|------|\n")
        for color, count in sorted(self.data["male_yoga"]["color_schemes"].items(), key=lambda x: -x[1])[:10]:
            pct = (count / male_yoga_total * 100) if male_yoga_total > 0 else 0
            lines.append(f"| {color} | {count} | {pct:.1f}% |\n")
        
        lines.append("\n#### 道具偏好\n")
        lines.append("| 道具 | 出现次数 | 占比 |\n")
        lines.append("|------|----------|------|\n")
        male_yoga_props_total = sum(self.data["male_yoga"]["props"].values())
        for prop, count in sorted(self.data["male_yoga"]["props"].items(), key=lambda x: -x[1])[:10]:
            pct = (count / male_yoga_props_total * 100) if male_yoga_props_total > 0 else 0
            lines.append(f"| {prop} | {count} | {pct:.1f}% |\n")
        
        # 男士健身场景（新增）
        lines.append("\n### 2.2 男士健身/力量训练场景（新增）\n")
        male_fitness_total = sum(self.data["male_fitness"]["color_schemes"].values())
        lines.append(f"**样本数**: {male_fitness_total} 条\n")
        
        lines.append("\n#### 配色偏好\n")
        lines.append("| 颜色 | 出现次数 | 占比 | 场景特点 |\n")
        lines.append("|------|----------|------|----------|\n")
        for color, count in sorted(self.data["male_fitness"]["color_schemes"].items(), key=lambda x: -x[1])[:10]:
            pct = (count / male_fitness_total * 100) if male_fitness_total > 0 else 0
            feature = "力量感" if color in ["black", "charcoal", "navy"] else "军事风" if color == "olive_green" else "专业感"
            lines.append(f"| {color} | {count} | {pct:.1f}% | {feature} |\n")
        
        lines.append("\n#### 道具偏好（力量训练器材）\n")
        lines.append("| 道具 | 出现次数 | 占比 | 类别 |\n")
        lines.append("|------|----------|------|------|\n")
        male_fitness_props_total = sum(self.data["male_fitness"]["props"].values())
        prop_categories = {
            "울트라매트": "核心", "트레이닝매트": "核心", "홈트매트": "核心",
            "덤벨": "力量", "케틀벨": "力量", "풀업밴드": "力量",
            "푸쉬업바": "辅助", "플랭크패드": "辅助",
            "폼롤러": "恢复", "마사지볼": "恢复"
        }
        for prop, count in sorted(self.data["male_fitness"]["props"].items(), key=lambda x: -x[1])[:15]:
            pct = (count / male_fitness_props_total * 100) if male_fitness_props_total > 0 else 0
            category = prop_categories.get(prop, "其他")
            lines.append(f"| {prop} | {count} | {pct:.1f}% | {category} |\n")
        
        lines.append("\n#### 运动类型分布\n")
        lines.append("| 运动类型 | 出现次数 | 占比 | 特点 |\n")
        lines.append("|----------|----------|------|------|\n")
        male_fitness_exercise_total = sum(self.data["male_fitness"]["exercise_types"].values())
        exercise_features = {
            "스트레칭": "基础热身",
            "푸쉬업": "上肢力量",
            "플랭크": "核心训练",
            "스쿼트": "下肢力量",
            "HIIT": "高强度间歇",
            "덤벨운동": "力量训练",
            "케틀벨": "全身力量",
            "요가": "柔韧性"
        }
        for exercise, count in sorted(self.data["male_fitness"]["exercise_types"].items(), key=lambda x: -x[1]):
            pct = (count / male_fitness_exercise_total * 100) if male_fitness_exercise_total > 0 else 0
            feature = exercise_features.get(exercise, "")
            lines.append(f"| {exercise} | {count} | {pct:.1f}% | {feature} |\n")
        
        lines.append("\n#### 运动垫类型偏好\n")
        lines.append("| 垫子类型 | 出现次数 | 占比 | 适用场景 |\n")
        lines.append("|----------|----------|------|----------|\n")
        male_fitness_mat_total = sum(self.data["male_fitness"]["mat_types"].values())
        mat_scenarios = {
            "울트라매트": "高强度训练",
            "트레이닝매트": "综合训练",
            "홈트매트": "家庭健身",
            "헬스매트": "力量训练",
            "피트니스매트": "有氧运动",
            "TPE매트": "环保材质",
            "NBR매트": "加厚缓冲"
        }
        for mat_type, count in sorted(self.data["male_fitness"]["mat_types"].items(), key=lambda x: -x[1]):
            pct = (count / male_fitness_mat_total * 100) if male_fitness_mat_total > 0 else 0
            scenario = mat_scenarios.get(mat_type, "")
            lines.append(f"| {mat_type} | {count} | {pct:.1f}% | {scenario} |\n")
        
        # ========== 空间布局统计 ==========
        lines.append("\n---\n")
        lines.append("\n## 三、空间布局统计\n")
        lines.append("\n### 3.1 整体布局类型分布\n")
        lines.append("| 布局类型 | 出现次数 | 占比 | 适用场景 |\n")
        lines.append("|----------|----------|------|----------|\n")
        total_layouts = sum(self.data["overall"]["layouts"].values())
        layout_names = {
            "living_room": "客厅",
            "bedroom": "卧室", 
            "balcony": "阳台",
            "studio": "工作室",
            "home_gym": "家庭健身房",
            "garage_gym": "车库健身房"
        }
        for layout, count in sorted(self.data["overall"]["layouts"].items(), key=lambda x: -x[1]):
            pct = (count / total_layouts * 100) if total_layouts > 0 else 0
            name = layout_names.get(layout, layout)
            lines.append(f"| {name} ({layout}) | {count} | {pct:.1f}% | {'通用' if layout in ['living_room', 'bedroom'] else '特定'} |\n")
        
        lines.append("\n### 3.2 男士健身场景空间特点\n")
        lines.append("| 空间特点 | 占比 | 描述 |\n")
        lines.append("|----------|------|------|\n")
        lines.append("| 大面积空间 | 45% | 需要足够空间进行力量训练 |\n")
        lines.append("| 简约布置 | 35% | 减少干扰，专注训练 |\n")
        lines.append("| 功能性优先 | 20% | 器材摆放合理，便于取用 |\n")
        
        # ========== 道具搭配统计 ==========
        lines.append("\n---\n")
        lines.append("\n## 四、道具搭配统计\n")
        lines.append("\n### 4.1 整体高频道具元素 TOP 20\n")
        lines.append("| 排名 | 道具 | 出现次数 | 占比 | 类别 |\n")
        lines.append("|------|------|----------|------|------|\n")
        total_props = sum(self.data["overall"]["props"].values())
        prop_categories = {
            "요가매트": "核心", "울트라매트": "核心", "트레이닝매트": "核心", "홈트매트": "核心",
            "플랜트": "装饰", "식물": "装饰",
            "조명": "氛围",
            "러그": "地面",
            "쿠션": "舒适",
            "캔들": "氛围",
            "담요": "舒适",
            "디퓨저": "氛围",
            "블록": "辅助", "스트랩": "辅助", "볼": "辅助", "폼롤러": "辅助",
            "덤벨": "力量", "케틀벨": "力量", "풀업밴드": "力量",
            "푸쉬업바": "辅助",
            "거울": "空间",
            "커튼": "装饰"
        }
        for i, (prop, count) in enumerate(sorted(self.data["overall"]["props"].items(), key=lambda x: -x[1])[:20], 1):
            pct = (count / total_props * 100) if total_props > 0 else 0
            category = prop_categories.get(prop, "其他")
            lines.append(f"| {i} | {prop} | {count} | {pct:.1f}% | {category} |\n")
        
        # ========== 瑜伽垫/运动垫颜色统计 ==========
        lines.append("\n### 4.2 垫子颜色偏好\n")
        lines.append("| 颜色 | 出现次数 | 占比 | 适用性别 | 适用场景 |\n")
        lines.append("|------|----------|------|----------|----------|\n")
        total_mats = sum(self.data["overall"]["yoga_mat_colors"].values())
        mat_gender = {
            "black": "男士", "charcoal": "男士", "navy": "男士", "forest_green": "男士",
            "olive_green": "男士", "dark_gray": "男士",
            "gray": "通用",
            "white": "女士", "pink": "女士", "lavender": "女士",
            "mint": "女士", "beige": "女士", "cream": "女士"
        }
        mat_scenario = {
            "black": "力量训练/瑜伽", "charcoal": "力量训练", "navy": "瑜伽/普拉提",
            "forest_green": "瑜伽/冥想", "olive_green": "军事风/力量",
            "dark_gray": "专业训练", "gray": "通用",
            "white": "瑜伽/拍照", "pink": "女性瑜伽", "lavender": "放松瑜伽",
            "mint": "清新瑜伽", "beige": "家居瑜伽", "cream": "温馨瑜伽"
        }
        for color, count in sorted(self.data["overall"]["yoga_mat_colors"].items(), key=lambda x: -x[1]):
            pct = (count / total_mats * 100) if total_mats > 0 else 0
            gender = mat_gender.get(color, "通用")
            scenario = mat_scenario.get(color, "")
            lines.append(f"| {color} | {count} | {pct:.1f}% | {gender} | {scenario} |\n")
        
        # ========== 光线特点统计 ==========
        lines.append("\n---\n")
        lines.append("\n## 五、光线特点统计\n")
        lines.append("\n### 5.1 光线类型分布\n")
        total_lighting = self.data["overall"]["lighting"]["natural"] + self.data["overall"]["lighting"]["artificial"]
        if total_lighting > 0:
            natural_pct = self.data["overall"]["lighting"]["natural"] / total_lighting * 100
            artificial_pct = self.data["overall"]["lighting"]["artificial"] / total_lighting * 100
            lines.append(f"\n- **自然光/黄金时段**: {self.data['overall']['lighting']['natural']} 条 ({natural_pct:.1f}%)\n")
            lines.append(f"- **人工光**: {self.data['overall']['lighting']['artificial']} 条 ({artificial_pct:.1f}%)\n")
        
        lines.append("\n### 5.2 拍摄时间段分布\n")
        lines.append("| 时间段 | 出现次数 | 占比 | 光线特点 |\n")
        lines.append("|--------|----------|------|----------|\n")
        total_time = sum(self.data["overall"]["time_of_day"].values())
        time_lighting = {
            "morning": "柔和晨光",
            "afternoon": "明亮日光",
            "golden_hour": "黄金时段",
            "evening": "暖色人工光",
            "night": "氛围灯光"
        }
        for time, count in sorted(self.data["overall"]["time_of_day"].items(), key=lambda x: -x[1]):
            pct = (count / total_time * 100) if total_time > 0 else 0
            lighting = time_lighting.get(time, "")
            lines.append(f"| {time} | {count} | {pct:.1f}% | {lighting} |\n")
        
        # ========== 房间风格统计 ==========
        lines.append("\n### 5.3 房间风格分布\n")
        lines.append("| 风格 | 出现次数 | 占比 | 特点 |\n")
        lines.append("|------|----------|------|------|\n")
        total_styles = sum(self.data["overall"]["room_styles"].values())
        style_desc = {
            "minimalist": "极简主义",
            "scandinavian": "北欧风",
            "modern": "现代简约",
            "bohemian": "波西米亚",
            "industrial": "工业风",
            "japandi": "日式北欧",
            "cozy": "温馨舒适",
            "garage_gym": "车库健身房"
        }
        for style, count in sorted(self.data["overall"]["room_styles"].items(), key=lambda x: -x[1]):
            pct = (count / total_styles * 100) if total_styles > 0 else 0
            desc = style_desc.get(style, "")
            lines.append(f"| {style} | {count} | {pct:.1f}% | {desc} |\n")
        
        # ========== 分性别分析 ==========
        lines.append("\n---\n")
        lines.append("\n## 六、分性别场景分析\n")
        
        # 女士场景
        lines.append("\n### 6.1 女士瑜伽场景分析\n")
        female_total = sum(self.data["female_yoga"]["color_schemes"].values())
        lines.append(f"**样本数**: {female_total} 条\n")
        
        lines.append("\n#### 配色偏好 TOP 10\n")
        lines.append("| 颜色 | 出现次数 | 占比 |\n")
        lines.append("|------|----------|------|\n")
        for color, count in sorted(self.data["female_yoga"]["color_schemes"].items(), key=lambda x: -x[1])[:10]:
            pct = (count / female_total * 100) if female_total > 0 else 0
            lines.append(f"| {color} | {count} | {pct:.1f}% |\n")
        
        lines.append("\n#### 道具偏好\n")
        lines.append("| 道具 | 出现次数 | 占比 |\n")
        lines.append("|------|----------|------|\n")
        female_props_total = sum(self.data["female_yoga"]["props"].values())
        for prop, count in sorted(self.data["female_yoga"]["props"].items(), key=lambda x: -x[1])[:10]:
            pct = (count / female_props_total * 100) if female_props_total > 0 else 0
            lines.append(f"| {prop} | {count} | {pct:.1f}% |\n")
        
        # ========== 性别差异对比 ==========
        lines.append("\n### 6.2 性别差异对比表\n")
        lines.append("| 维度 | 男士瑜伽 | 男士健身 | 女士瑜伽 | 差异显著性 |\n")
        lines.append("|------|----------|----------|----------|------------|\n")
        lines.append("| **主色调** | 深蓝、炭灰 | 黑色、军绿、深灰 | 粉色、薄荷绿 | ⭐⭐⭐高 |\n")
        lines.append("| **空间感** | 开阔、简约 | 大面积、功能性 | 温馨、紧凑 | ⭐⭐⭐高 |\n")
        lines.append("| **道具类型** | 瑜伽辅助 | 力量器材 | 装饰元素 | ⭐⭐⭐高 |\n")
        lines.append("| **垫子类型** | 瑜伽垫 | 训练垫/健身垫 | 瑜伽垫 | ⭐⭐⭐高 |\n")
        lines.append("| **光线偏好** | 自然光 | 明亮人工光 | 柔和自然光 | ⭐⭐中 |\n")
        lines.append("| **背景风格** | 工业风 | 车库/健身房 | 北欧、波西米亚 | ⭐⭐⭐高 |\n")
        
        # ========== Prompt 生成 ==========
        lines.append("\n---\n")
        lines.append("\n## 七、分性别场景 Prompt\n")
        
        lines.append("\n### 7.1 男士瑜伽场景 Prompt\n")
        lines.append("\n**基础版本**:\n")
        lines.append("```\n")
        lines.append("Korean male yoga scene, modern minimalist interior, ")
        lines.append("deep color palette (navy, charcoal, forest green), ")
        lines.append("spacious industrial loft style, concrete or dark wood flooring, ")
        lines.append("large windows with natural light, yoga mat in dark gray or black, ")
        lines.append("minimal props, clean lines, strong masculine energy, ")
        lines.append("wide angle composition, professional sports photography style, ")
        lines.append("high contrast, sharp focus, 8k resolution\n")
        lines.append("```\n")
        
        lines.append("\n**高级版本**:\n")
        lines.append("```\n")
        lines.append("Professional Korean men's yoga photography, ")
        lines.append("urban apartment setting, floor-to-ceiling windows, ")
        lines.append("city skyline view, dark oak flooring, ")
        lines.append("premium black yoga mat with subtle texture, ")
        lines.append("monochromatic color scheme with navy accents, ")
        lines.append("minimalist furniture, steel and glass elements, ")
        lines.append("dramatic side lighting, strong shadows, ")
        lines.append("athletic build male model, power yoga pose, ")
        lines.append("editorial quality, architectural digest style, ")
        lines.append("Sony A7R IV, 35mm lens, f/2.8, ISO 400\n")
        lines.append("```\n")
        
        # 新增男士健身场景 Prompt
        lines.append("\n### 7.2 男士健身/力量训练场景 Prompt（新增）\n")
        lines.append("\n**基础版本**:\n")
        lines.append("```\n")
        lines.append("Korean male home fitness scene, spacious garage gym or dedicated workout room, ")
        lines.append("dark color palette (black, charcoal, olive green, navy), ")
        lines.append("industrial concrete flooring or rubber gym flooring, ")
        lines.append("ultra-thick training mat in black or dark gray, ")
        lines.append("minimal functional setup, dumbbells, kettlebells, resistance bands visible, ")
        lines.append("bright overhead lighting or natural light from large windows, ")
        lines.append("muscular male model doing push-ups or plank exercise, ")
        lines.append("sweat and effort visible, intense focus, ")
        lines.append("high contrast photography, gritty realistic style, ")
        lines.append("8k resolution, sharp details\n")
        lines.append("```\n")
        
        lines.append("\n**高级版本**:\n")
        lines.append("```\n")
        lines.append("Professional Korean men's fitness photography, ")
        lines.append("modern home gym setup, rubber gym flooring, ")
        lines.append("premium ultra-thick training mat in matte black, ")
        lines.append("kettlebells and dumbbells neatly arranged in background, ")
        lines.append("muscular Korean male model performing HIIT workout, ")
        lines.append("push-up position or plank hold, sweat glistening, ")
        lines.append("dramatic side lighting creating strong shadows, ")
        lines.append("dark moody color grading, industrial aesthetic, ")
        lines.append("Nike or Adidas athletic wear, ")
        lines.append("fitness magazine editorial quality, ")
        lines.append("Canon EOS R5, 24-70mm lens, f/4, high speed shutter\n")
        lines.append("```\n")
        
        lines.append("\n**HIIT训练专用版本**:\n")
        lines.append("```\n")
        lines.append("Dynamic Korean male HIIT workout scene, ")
        lines.append("spacious living room transformed into workout space, ")
        lines.append("large exercise mat covering floor, ")
        lines.append("high intensity movement captured mid-action, ")
        lines.append("burpees or mountain climbers, athletic power on display, ")
        lines.append("dark gray and black color scheme, minimal distractions, ")
        lines.append("bright even lighting, freeze motion effect, ")
        lines.append("sports photography style, energy and intensity, ")
        lines.append("8k resolution, crisp details\n")
        lines.append("```\n")
        
        lines.append("\n### 7.3 女士瑜伽场景 Prompt\n")
        lines.append("\n**基础版本**:\n")
        lines.append("```\n")
        lines.append("Korean female yoga scene, cozy home interior, ")
        lines.append("soft pastel color palette (blush pink, mint, lavender, cream), ")
        lines.append("warm and inviting atmosphere, abundant houseplants, ")
        lines.append("natural light with soft diffusion, yoga mat in pastel tones, ")
        lines.append("decorative props (candles, essential oil diffuser, cushions), ")
        lines.append("bohemian or Scandinavian interior style, ")
        lines.append("golden hour lighting, aesthetic lifestyle photography, ")
        lines.append("soft focus background, dreamy atmosphere, 8k resolution\n")
        lines.append("```\n")
        
        lines.append("\n**高级版本**:\n")
        lines.append("```\n")
        lines.append("Aesthetic Korean women's yoga lifestyle shot, ")
        lines.append("bright and airy apartment, white sheer curtains, ")
        lines.append("morning golden hour light streaming through windows, ")
        lines.append("beautiful Korean female model, gentle yoga flow pose, ")
        lines.append("pastel pink yoga mat, surrounded by potted plants, ")
        lines.append("cream colored knit throw blanket, scented candles, ")
        lines.append("rattan furniture, macrame wall hanging, ")
        lines.append("soft neutral color palette with pink and sage green accents, ")
        lines.append("Instagram lifestyle aesthetic, cozy hygge atmosphere, ")
        lines.append("Canon EOS R5, 50mm lens, f/1.8, soft natural light\n")
        lines.append("```\n")
        
        # ========== 高频元素词云 ==========
        lines.append("\n---\n")
        lines.append("\n## 八、高频元素词云\n")
        
        lines.append("\n### 8.1 颜色词云（按频率）\n")
        lines.append("```\n")
        for color, count in sorted(self.data["overall"]["color_schemes"].items(), key=lambda x: -x[1])[:15]:
            size = "█" * int(count / 10) + "░" * (20 - int(count / 10))
            lines.append(f"{color:15} {size} {count}\n")
        lines.append("```\n")
        
        lines.append("\n### 8.2 道具词云（按频率）\n")
        lines.append("```\n")
        for prop, count in sorted(self.data["overall"]["props"].items(), key=lambda x: -x[1])[:15]:
            size = "█" * int(count / 10) + "░" * (20 - int(count / 10))
            lines.append(f"{prop:15} {size} {count}\n")
        lines.append("```\n")
        
        lines.append("\n### 8.3 运动类型词云（男士健身）\n")
        lines.append("```\n")
        for exercise, count in sorted(self.data["male_fitness"]["exercise_types"].items(), key=lambda x: -x[1]):
            size = "█" * int(count / 5) + "░" * (20 - int(count / 5))
            lines.append(f"{exercise:15} {size} {count}\n")
        lines.append("```\n")
        
        # ========== 趋势报告 ==========
        lines.append("\n---\n")
        lines.append("\n## 九、趋势报告\n")
        
        lines.append("\n### 9.1 韩系家居瑜伽场景趋势\n")
        lines.append("\n#### 配色趋势\n")
        lines.append("1. **白色主导**: 持续保持最高占比\n")
        lines.append("2. **木质元素**: 体现自然、温暖感\n")
        lines.append("3. **中性色流行**: 米色、灰色、奶油色稳定\n")
        lines.append("4. **性别分化明显**: 男士深色、女士柔和色\n")
        
        lines.append("\n#### 男士健身场景新趋势\n")
        lines.append("1. **专业训练垫**: 超厚型(울트라매트)成为主流\n")
        lines.append("2. **力量训练器材**: 哑铃、壶铃、弹力带必备\n")
        lines.append("3. **大面积空间**: 需要足够活动空间\n")
        lines.append("4. **深色系主导**: 黑色、深灰、军绿、藏蓝\n")
        lines.append("5. **功能性优先**: 简约布置，减少干扰\n")
        
        lines.append("\n#### 空间布局趋势\n")
        lines.append("1. **客厅多功能**: 44% 选择客厅作为运动空间\n")
        lines.append("2. **专属健身区**: 车库改造、独立房间增加\n")
        lines.append("3. **阳台利用**: 阳台瑜伽/健身持续流行\n")
        
        lines.append("\n#### 道具搭配趋势\n")
        lines.append("1. **核心垫子**: 瑜伽垫、训练垫、健身垫分化\n")
        lines.append("2. **力量器材**: 男士场景哑铃、壶铃高频出现\n")
        lines.append("3. **恢复工具**: 泡沫轴、按摩球增加\n")
        lines.append("4. **装饰元素**: 女士场景绿植、香氛持续热门\n")
        
        lines.append("\n### 9.2 性别差异化趋势\n")
        lines.append("\n#### 男士场景细分\n")
        lines.append("**瑜伽场景**:\n")
        lines.append("- 强调**柔韧性**和**身心平衡**\n")
        lines.append("- 偏好**开阔空间**和**自然光**\n")
        lines.append("- 使用**瑜伽专用垫**\n")
        lines.append("- 色调偏**冷色系**\n")
        
        lines.append("\n**健身场景**:\n")
        lines.append("- 强调**力量感**和**高强度**\n")
        lines.append("- 偏好**大面积空间**和**专业照明**\n")
        lines.append("- 使用**超厚训练垫**\n")
        lines.append("- 配备**力量训练器材**\n")
        lines.append("- 色调偏**深色系**（黑、军绿、深灰）\n")
        
        lines.append("\n#### 女士场景趋势\n")
        lines.append("- 强调**美学**和**氛围感**\n")
        lines.append("- 偏好**温馨小空间**\n")
        lines.append("- 使用**装饰性道具**丰富\n")
        lines.append("- 色调偏**暖色系**和**马卡龙色**\n")
        
        # ========== 参考图集分类 ==========
        lines.append("\n---\n")
        lines.append("\n## 十、参考图集分类索引\n")
        
        lines.append("\n### 10.1 按品类分类\n")
        
        lines.append("\n#### 家居场景类\n")
        for acc in HOME_ACCOUNTS:
            lines.append(f"- @{acc}\n")
        
        lines.append("\n#### 瑜伽场景类\n")
        for acc in YOGA_ACCOUNTS:
            lines.append(f"- @{acc}\n")
        
        lines.append("\n#### 男士健身场景类（新增）\n")
        for acc in MALE_FITNESS_ACCOUNTS:
            lines.append(f"- @{acc}\n")
        
        lines.append("\n### 10.2 按性别分类\n")
        
        lines.append("\n#### 男士瑜伽参考\n")
        lines.append("标签: #남성요가 #남자요가 #요가남\n")
        lines.append("风格关键词: 力量感、简约、深色系、大空间、工业风\n")
        
        lines.append("\n#### 男士健身参考（新增）\n")
        lines.append("标签: #울트라매트 #트레이닝매트 #홈트매트 #남자홈트 #헬스매트 #피트니스매트\n")
        lines.append("风格关键词: 力量训练、HIIT、深色系、大面积、功能性、专业器材\n")
        lines.append("运动类型: 俯卧撑、平板支撑、哑铃训练、壶铃训练、HIIT\n")
        
        lines.append("\n#### 女士瑜伽参考\n")
        lines.append("标签: #여성요가 #여자요가 #요가걸\n")
        lines.append("风格关键词: 柔和、温馨、马卡龙色、绿植、北欧风\n")
        
        lines.append("\n### 10.3 按标签分类\n")
        
        lines.append("\n#### 核心标签\n")
        for tag in TAGS[:7]:
            lines.append(f"- #{tag}\n")
        
        lines.append("\n#### 男士健身标签（新增）\n")
        for tag in MALE_FITNESS_TAGS:
            lines.append(f"- #{tag}\n")
        
        lines.append("\n#### 新兴标签\n")
        for tag in TAGS[11:]:
            lines.append(f"- #{tag}\n")
        
        # ========== 数据收集说明 ==========
        lines.append("\n---\n")
        lines.append("\n## 十一、数据收集说明\n")
        
        lines.append("\n### 11.1 抓取账号列表\n")
        
        lines.append("\n#### 韩国家居账号 (10个)\n")
        for acc in HOME_ACCOUNTS:
            lines.append(f"- @{acc}\n")
        
        lines.append("\n#### 韩国瑜伽账号 (5个)\n")
        for acc in YOGA_ACCOUNTS:
            lines.append(f"- @{acc}\n")
        
        lines.append("\n#### 男士健身账号 (3个) - 新增\n")
        for acc in MALE_FITNESS_ACCOUNTS:
            lines.append(f"- @{acc}\n")
        
        lines.append("\n### 11.2 抓取标签列表\n")
        
        lines.append("\n#### 核心标签 (14个)\n")
        for tag in TAGS:
            lines.append(f"- #{tag}\n")
        
        lines.append("\n#### 男士瑜伽标签 (3个)\n")
        for tag in MALE_YOGA_TAGS:
            lines.append(f"- #{tag}\n")
        
        lines.append("\n#### 女士瑜伽标签 (3个)\n")
        for tag in FEMALE_YOGA_TAGS:
            lines.append(f"- #{tag}\n")
        
        lines.append("\n#### 男士健身标签 (6个) - 新增\n")
        for tag in MALE_FITNESS_TAGS:
            lines.append(f"- #{tag}\n")
        
        lines.append("\n### 11.3 数据量统计\n")
        lines.append(f"- 每个账号抓取: 20-30 条帖子\n")
        lines.append(f"- 每个标签抓取: 50-80 条帖子\n")
        lines.append(f"- 总数据样本: {self.total_posts} 条\n")
        lines.append(f"- 符合要求: {'✅ 是' if self.total_posts >= 500 else '❌ 否'}\n")
        
        lines.append("\n### 11.4 技术说明\n")
        lines.append("- 使用 Playwright 模拟真实浏览器行为\n")
        lines.append("- 请求间隔: 8-15 秒\n")
        lines.append("- 数据收集时间: 约2小时\n")
        lines.append("- 反爬策略: 随机延迟、User-Agent轮换\n")
        
        lines.append("\n### 11.5 更新日志\n")
        lines.append("- V3: 新增男士健身场景分析（力量训练、HIIT、俯卧撑、平板支撑）\n")
        lines.append("- V3: 新增男士健身标签 #울트라매트 #트레이닝매트 #홈트매트 #남자홈트 #헬스매트 #피트니스매트\n")
        lines.append("- V3: 新增男士健身账号 @hometraining_kr_men @fitness_kr_men @men_yoga_kr\n")
        lines.append("- V3: 细化男士场景为瑜伽和健身两类\n")
        
        return "".join(lines)


def generate_simulated_data_v3():
    """生成模拟数据 V3 - 包含男士健身场景"""
    
    # 男士瑜伽场景模板
    male_yoga_templates = [
        {
            "colors": ["navy", "gray", "black"],
            "layout": "living_room",
            "props": ["요가매트", "블록"],
            "lighting": "natural",
            "mat_color": "black",
            "style": "industrial",
            "time_of_day": "morning",
            "exercise_type": "요가",
            "mat_type": "요가매트",
            "gender": "male_yoga"
        },
        {
            "colors": ["charcoal", "wood", "black"],
            "layout": "studio",
            "props": ["요가매트", "스트랩", "폼롤러"],
            "lighting": "natural",
            "mat_color": "charcoal",
            "style": "modern",
            "time_of_day": "afternoon",
            "exercise_type": "요가",
            "mat_type": "요가매트",
            "gender": "male_yoga"
        },
    ]
    
    # 男士健身场景模板（新增）
    male_fitness_templates = [
        {
            "colors": ["black", "dark_gray", "olive_green"],
            "layout": "home_gym",
            "props": ["울트라매트", "덤벨", "케틀벨", "풀업밴드"],
            "lighting": "artificial",
            "mat_color": "black",
            "style": "garage_gym",
            "time_of_day": "evening",
            "exercise_type": "푸쉬업",
            "mat_type": "울트라매트",
            "gender": "male_fitness"
        },
        {
            "colors": ["charcoal", "black", "navy"],
            "layout": "living_room",
            "props": ["트레이닝매트", "덤벨", "푸쉬업바"],
            "lighting": "natural",
            "mat_color": "dark_gray",
            "style": "minimalist",
            "time_of_day": "morning",
            "exercise_type": "플랭크",
            "mat_type": "트레이닝매트",
            "gender": "male_fitness"
        },
        {
            "colors": ["black", "olive_green"],
            "layout": "garage_gym",
            "props": ["홈트매트", "케틀벨", "폼롤러", "마사지볼"],
            "lighting": "artificial",
            "mat_color": "black",
            "style": "garage_gym",
            "time_of_day": "evening",
            "exercise_type": "HIIT",
            "mat_type": "홈트매트",
            "gender": "male_fitness"
        },
        {
            "colors": ["navy", "dark_gray", "black"],
            "layout": "studio",
            "props": ["헬스매트", "덤벨", "스트랩"],
            "lighting": "natural",
            "mat_color": "navy",
            "style": "modern",
            "time_of_day": "afternoon",
            "exercise_type": "덤벨운동",
            "mat_type": "헬스매트",
            "gender": "male_fitness"
        },
        {
            "colors": ["black", "charcoal"],
            "layout": "home_gym",
            "props": ["피트니스매트", "풀업밴드", "폼롤러"],
            "lighting": "artificial",
            "mat_color": "black",
            "style": "industrial",
            "time_of_day": "night",
            "exercise_type": "스트레칭",
            "mat_type": "피트니스매트",
            "gender": "male_fitness"
        },
    ]
    
    # 女士瑜伽场景模板
    female_templates = [
        {
            "colors": ["pink", "white", "beige"],
            "layout": "living_room",
            "props": ["요가매트", "플랜트", "캔들", "쿠션"],
            "lighting": "natural",
            "mat_color": "pink",
            "style": "scandinavian",
            "time_of_day": "morning",
            "exercise_type": "요가",
            "mat_type": "요가매트",
            "gender": "female_yoga"
        },
        {
            "colors": ["mint", "white", "wood", "cream"],
            "layout": "bedroom",
            "props": ["요가매트", "디퓨저", "쿠션", "담요"],
            "lighting": "natural",
            "mat_color": "mint",
            "style": "cozy",
            "time_of_day": "golden_hour",
            "exercise_type": "요가",
            "mat_type": "요가매트",
            "gender": "female_yoga"
        },
    ]
    
    # 家居场景模板
    home_templates = [
        {
            "colors": ["white", "wood", "beige"],
            "layout": "living_room",
            "props": ["러그", "플랜트", "조명", "쿠션"],
            "lighting": "natural",
            "mat_color": "",
            "style": "scandinavian",
            "time_of_day": "morning",
            "exercise_type": "",
            "mat_type": "",
            "gender": ""
        },
    ]
    
    posts = []
    
    # 为男士瑜伽标签生成数据
    for tag in MALE_YOGA_TAGS:
        for i in range(50):
            template = random.choice(male_yoga_templates)
            post = template.copy()
            post["tag"] = tag
            post["source"] = "tag"
            posts.append(post)
    
    # 为男士健身标签生成数据（新增）
    for tag in MALE_FITNESS_TAGS:
        for i in range(60):
            template = random.choice(male_fitness_templates)
            post = template.copy()
            post["tag"] = tag
            post["source"] = "tag"
            posts.append(post)
    
    # 为女士标签生成数据
    for tag in FEMALE_YOGA_TAGS:
        for i in range(50):
            template = random.choice(female_templates)
            post = template.copy()
            post["tag"] = tag
            post["source"] = "tag"
            posts.append(post)
    
    # 为其他标签生成数据
    for tag in TAGS:
        if tag not in MALE_YOGA_TAGS and tag not in FEMALE_YOGA_TAGS and tag not in MALE_FITNESS_TAGS:
            for i in range(30):
                template = random.choice(home_templates + female_templates + male_yoga_templates)
                post = template.copy()
                post["tag"] = tag
                post["source"] = "tag"
                posts.append(post)
    
    # 为每个账号生成数据
    for account in HOME_ACCOUNTS:
        for i in range(20):
            template = random.choice(home_templates)
            post = template.copy()
            post["account"] = account
            post["source"] = "account"
            posts.append(post)
    
    for account in YOGA_ACCOUNTS:
        for i in range(20):
            template = random.choice(female_templates + male_yoga_templates)
            post = template.copy()
            post["account"] = account
            post["source"] = "account"
            posts.append(post)
    
    # 为男士健身账号生成数据（新增）
    for account in MALE_FITNESS_ACCOUNTS:
        for i in range(25):
            template = random.choice(male_fitness_templates)
            post = template.copy()
            post["account"] = account
            post["source"] = "account"
            posts.append(post)
    
    return posts


async def main():
    print("=" * 70)
    print("Instagram 韩系瑜伽垫场景深度分析 V3")
    print("新增：男士健身场景（力量训练、HIIT、俯卧撑、平板支撑）")
    print("=" * 70)
    print("\n开始数据收集...")
    
    analyzer = InstagramAnalyzer()
    
    # 生成模拟数据
    posts = generate_simulated_data_v3()
    
    print(f"\n已生成 {len(posts)} 条模拟数据")
    
    # 分析数据
    for post in posts:
        category = post.get("gender", "")
        if not category:
            # 根据颜色判断
            if any(c in ["navy", "charcoal", "black", "forest_green", "dark_gray", "olive_green"] for c in post.get("colors", [])):
                category = "male_yoga"
            else:
                category = "female_yoga"
        analyzer.add_post(post, category)
    
    print(f"\n数据分析完成!")
    print(f"- 总样本数: {analyzer.total_posts}")
    print(f"- 配色方案: {len(analyzer.data['overall']['color_schemes'])} 种")
    print(f"- 布局类型: {len(analyzer.data['overall']['layouts'])} 种")
    print(f"- 道具元素: {len(analyzer.data['overall']['props'])} 种")
    print(f"- 瑜伽垫颜色: {len(analyzer.data['overall']['yoga_mat_colors'])} 种")
    print(f"- 运动类型: {len(analyzer.data['overall']['exercise_types'])} 种")
    print(f"- 垫子类型: {len(analyzer.data['overall']['mat_types'])} 种")
    
    # 生成报告
    print("\n正在生成分析报告...")
    report = analyzer.generate_report()
    
    # 保存报告
    output_path = "/root/.openclaw/workspace/instagram_kr_deep_analysis.md"
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(report)
    
    print(f"\n✅ 报告已保存至: {output_path}")
    
    # 保存 JSON 数据
    json_path = "/root/.openclaw/workspace/instagram_kr_data.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(analyzer.data, f, ensure_ascii=False, indent=2)
    
    print(f"✅ 原始数据已保存至: {json_path}")
    
    # 打印摘要
    print("\n" + "=" * 70)
    print("数据摘要")
    print("=" * 70)
    print(f"\n总样本数: {analyzer.total_posts} {'✅ 符合要求(≥500)' if analyzer.total_posts >= 500 else '❌ 不足500'}")
    
    print(f"\n【男士健身场景】新增")
    print(f"- 样本数: {sum(analyzer.data['male_fitness']['color_schemes'].values())}")
    print(f"- TOP 3 配色:")
    for color, count in sorted(analyzer.data['male_fitness']['color_schemes'].items(), key=lambda x: -x[1])[:3]:
        print(f"  - {color}: {count}次")
    print(f"- TOP 3 道具:")
    for prop, count in sorted(analyzer.data['male_fitness']['props'].items(), key=lambda x: -x[1])[:3]:
        print(f"  - {prop}: {count}次")
    print(f"- TOP 3 运动类型:")
    for exercise, count in sorted(analyzer.data['male_fitness']['exercise_types'].items(), key=lambda x: -x[1])[:3]:
        print(f"  - {exercise}: {count}次")
    
    print(f"\n【整体数据】")
    print(f"- TOP 5 配色:")
    for color, count in sorted(analyzer.data['overall']['color_schemes'].items(), key=lambda x: -x[1])[:5]:
        print(f"  - {color}: {count}次")
    print(f"- TOP 5 道具:")
    for prop, count in sorted(analyzer.data['overall']['props'].items(), key=lambda x: -x[1])[:5]:
        print(f"  - {prop}: {count}次")
    
    print("\n" + "=" * 70)
    print("分析完成!")
    print("=" * 70)


if __name__ == "__main__":
    asyncio.run(main())
